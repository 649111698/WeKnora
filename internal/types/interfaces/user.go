...
type UserService interface {
	...
	// are enabled. SSO credentials are tenant-level: the tenant is resolved
	// from tenantParam (?t=<tenant id>, optional) then the request Host
	// against tenant login_domain, falling back to the single tenant that
	// has the provider configured (single-company deployments).
	GetSSOStatus(ctx context.Context, tenantParam, host string) (*types.SSOStatusResponse, error)
	// LoginWithWeComCode completes WeCom in-browser OAuth login (JIT-provisioning guest accounts).
	LoginWithWeComCode(ctx context.Context, code, tenantParam, host string, provisioning types.TenantProvisioningMode) (*types.OIDCCallbackResponse, error)
	// LoginWithFeishuCode completes Feishu in-browser OAuth login (JIT-provisioning guest accounts).
	LoginWithFeishuCode(ctx context.Context, code, tenantParam, host string, provisioning types.TenantProvisioningMode) (*types.OIDCCallbackResponse, error)
	...
	GetSSOWatermark(ctx context.Context, tenantParam, host string) types.WatermarkConfig
	...
	GetSSODomainVerifyText(ctx context.Context, tenantParam, host string) string
