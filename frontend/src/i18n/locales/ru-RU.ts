export default {
  usageReport: {
    navLabel: 'Отчёт об использовании',
    title: 'Ежедневный отчёт об использовании',
    desc: 'Каждый день в 09:00 сводится использование участниками за предыдущий день (норма = ≥1 вход и ≥2 диалога) и отправляется в WeCom согласно настройкам.',
    enable: 'Включить ежедневный отчёт',
    enableHint: 'При выключении планировщик пропускает это пространство',
    pushToWeCom: 'Отправлять в WeCom',
    pushToWeComHint: 'Требуется собственное приложение WeCom (SSO) и выбранные получатели',
    remindUnqualified: 'Напоминать не выполнившим норму',
    remindUnqualifiedHint: 'После отправки отчёта каждый не выполнивший норму участник WeCom получает персональную карточку-напоминание',
    recipients: 'Получатели',
    recipientsHint: 'Выбирать можно только участников, созданных через WeCom (сообщения приложения — только WeCom)',
    recipientsPlaceholder: 'Выберите участников для уведомления',
    rule: 'Норма: ≥1 вход и ≥2 диалога за вчера. Отчёт включает итоги, детали по участникам и сводку.',
    testNow: 'Отправить тестовый отчёт',
    testSent: 'Тестовый отчёт создан и отправлен',
    testFailed: 'Не удалось отправить тест',
    testResultTitle: 'Предпросмотр отчёта за {date}',
    cardTotal: 'Всего {n}',
    cardQualified: 'Норма {n}',
    cardUnqualified: 'Не выполнено {n}',
    cardMessages: 'Сообщений {n}',
    loadFailed: 'Не удалось загрузить настройки',
    saveFailed: 'Не удалось сохранить',
  },

  tenantSSO: {
      navLabel: 'SSO и водяной знак',
      loginDomain: {
        title: 'Выделенный домен входа',
        description: 'Привяжите выделенный домен входа к этому пространству (обязательно). Домен — единственный способ различать пространства: посетители с этого домена автоматически получают SSO и водяной знак этого пространства. Настройте DNS домена на этот сервер и выпустите TLS-сертификат. Домены уникальны для каждого пространства.',
        label: 'Домен входа',
        placeholder: 'напр. sso.your-company.com (можно с портом)',
        entryHint: 'Точка входа пространства',
      },
      guide: {
        wecom: {
          title: 'URL-адреса консоли WeCom',
          description: 'Заполните сгенерированные ниже значения в соответствующих полях консоли WeCom, чтобы завершить настройку входа приложения.',
          homeUrl: 'URL главной страницы приложения (Управление приложениями → Свое приложение → Главная)',
          trustedDomain: 'Доверенный домен (веб-авторизация и JS-SDK)',
          verifyNote: 'Когда WeCom запросит проверку доверенного домена, выберите проверку файлом и вставьте содержимое скачанного WW_verify_*.txt в поле «Текст проверки домена» выше — сайт автоматически ответит на запрос WeCom.',
        },
        feishu: {
          redirectUrl: 'Redirect URL веб-авторизации (платформа Feishu → Настройки безопасности)',
        },
      },
      wecom: {
        title: 'SSO WeCom',
        description: 'Учётные данные самописного приложения WeCom этого пространства. После заполнения CorpID и Secret участники, открывающие вход в браузере WeCom, входят автоматически; при первом входе создаётся гостевая учётная запись и добавляется в это пространство. Условия: доверенный домен веб-авторизации приложения охватывает сайт и участники входят в область видимости приложения.',
        corpId: 'Corp ID',
        corpIdPlaceholder: 'Моя компания → Информация → Corp ID',
        corpSecret: 'Secret приложения',
        corpSecretPlaceholder: 'Secret самописного приложения WeCom',
        agentId: 'AgentId',
        agentIdPlaceholder: 'AgentId приложения (необязательно, рекомендуется)',
        domainVerifyText: 'Текст проверки домена',
        domainVerifyPlaceholder: 'Содержимое файла WW_verify_*.txt из консоли WeCom',
        domainVerifyHint: 'Сайт будет отвечать на запросы /WW_verify_*.txt этим текстом — файл размещать вручную не нужно.',
      },
      feishu: {
        title: 'SSO Feishu',
        description: 'Учётные данные самописного приложения Feishu этого пространства. После заполнения App ID и App Secret вход выполняется автоматически во встроенном браузере Feishu. Условия: включите веб-авторизацию и укажите redirect URL https://<домен>/login.',
        appId: 'App ID',
        appIdPlaceholder: 'App ID приложения на платформе Feishu',
        appSecret: 'App Secret',
        appSecretPlaceholder: 'App Secret приложения на платформе Feishu',
      },
      kingdee: {
        title: 'SSO Kingdee Cosmic',
        description: 'Использовать портал Kingdee Cosmic как источник удостоверений (Cosmic V6.0.015+, OAuth2.0 authorization code). Зарегистрируйте системный код в «System Service Cloud → OpenAPI → Third-party App» на стороне Cosmic и авторизуйте API authen/getUserInfo; сотрудники входят через меню портала Cosmic, гостевая учётная запись создаётся при первом входе.',
        baseUrl: 'Адрес сайта Cosmic',
        baseUrlPlaceholder: 'напр. https://kde.erp.com (без косой черты в конце)',
        appClientId: 'Код системы (app_client_id)',
        appClientIdPlaceholder: 'Системный код стороннего приложения Cosmic',
        appSecret: 'appSecret',
        appSecretPlaceholder: 'Пользовательский appSecret из учётных данных Cosmic',
        appSecretHint: 'Обязателен для автономных развёртываний: вместе с двумя полями ниже обменивается на access_token перед вызовом API пользователя.',
        proxyUsername: 'Имя прокси-пользователя (username)',
        proxyUsernamePlaceholder: 'username из диалога Cosmic «Получить пример токена»',
        accountId: 'ID дата-центра (accountId)',
        accountIdPlaceholder: 'accountId из диалога Cosmic «Получить пример токена»',
        tokenModeHint: 'Все три поля выше (appSecret / username / accountId) показаны в Cosmic «Стороннее приложение → Получить пример токена». Для автономных развёртываний они обязательны; если хотя бы одно пусто, система возвращается к прямому режиму code, который большинство частных развёртываний отклоняет.',
        callbackUrl: 'Callback-адрес для SSO-белого списка',
        menuUrl: 'Ссылка автологина портала Cosmic',
        menuUrlHint: 'Настройте в Cosmic Меню / Быстрый запуск / Внешняя ссылка; вошедшие в Cosmic пользователи попадают в систему без повторного входа и по умолчанию оказываются на странице чата.',
        menuUrlFrameHint: 'Если Cosmic встраивает эту ссылку в iframe, задайте для контейнера frontend переменную FRAME_ALLOWED_ORIGINS=<origin портала Cosmic, напр. https://portal.example.com>; иначе встраивание блокируется политикой основного сайта (X-Frame-Options). При открытии в новом окне не требуется.',
      ...
      watermark: {
        title: 'Водяной знак пространства',
        description: 'Водяной знак для отслеживания утечек, действующий только в этом пространстве: при включении все страницы пространства получают текстовую накладку. Другие пространства не затрагиваются.',
        enabled: 'Включить водяной знак',
        text: 'Текст водяного знака',
        textPlaceholder: 'Поддерживается плейсхолдер {username}; пусто — только имя пользователя',
        textHint: '{username} заменяется именем вошедшего пользователя.',
      },
      keepSecretPlaceholder: 'Задан (оставьте пустым, чтобы не менять)',
      loadFailed: 'Не удалось загрузить конфигурацию SSO',
      ...
      branding: {
        ...
        description: 'Настройте приветствие, тексты страницы входа и логотип рабочего пространства. Пустые поля используют значения по умолчанию. Логотип можно загрузить напрямую (применяется сразу) или указать URL изображения (https:// или с префиксом /api).',
        ...
        uploadButton: 'Загрузить логотип',
        uploadSuccess: 'Логотип загружен и применён',
        uploadFailed: 'Не удалось загрузить логотип',
        uploadTooLarge: 'Изображение логотипа должно быть меньше 2 МБ'
      },
  },
  ...
  tenantMember: {
    ...
    directCreate: {
      button: 'Создать участника',
      dialogTitle: 'Создать участника (новая учётная запись)',
      nameLabel: 'Имя',
      namePlaceholder: 'Введите настоящее имя',
      nameRequired: 'Имя должно быть от 2 до 50 символов',
      usernameLabel: 'Имя пользователя',
      usernamePlaceholder: '2-50 символов',
      usernameRequired: 'Имя пользователя: 2-50 символов',
      passwordLabel: 'Начальный пароль',
      passwordPlaceholder: '8-32 символа, буквы и цифры',
      passwordPolicy: 'Пароль: 8-32 символа, минимум одна буква и одна цифра',
      submit: 'Создать',
      success: 'Участник создан и добавлен в пространство',
    ...
    editName: {
      ...
      required: 'Имя должно быть от 2 до 50 символов',
      saved: 'Имя обновлено',
      saveFailed: 'Не удалось обновить имя, повторите попытку',
    },
    ...
    agentAccess: {
      button: 'Доступ к агентам',
      dialogTitle: 'Доступ к агентам — {name}',
      description: 'Ограничьте, какие пользовательские агенты рабочего пространства участник может использовать в чате. Встроенные и общие агенты не затрагиваются.',
      modeAll: 'Все агенты (по умолчанию)',
      modeSelected: 'Только выбранные агенты',
      selectPlaceholder: 'Выберите разрешённых агентов',
      agentCount: 'Доступно: {total}, выбрано: {picked}',
      restrictedTag: 'Доступ к агентам ограничен',
      saved: 'Доступ к агентам обновлён',
      saveError: 'Не удалось обновить доступ к агентам',
      loadError: 'Не удалось загрузить список агентов'
    },
    defaultAgents: {
      label: 'Агенты по умолчанию для новых участников',
      modeAll: 'Все (по умолчанию)',
      modeSelected: 'Только выбранные агенты',
      selectPlaceholder: 'Выберите агентов, доступных по умолчанию',
      saved: 'Агенты по умолчанию обновлены',
      saveFailed: 'Не удалось обновить агентов по умолчанию'
    },
    add: {
      button: 'Пригласить',
  ...
  modelSettings: {
    ...
    modelLock: {
      title: 'Закрепить модель чата',
      description: 'Скрыть выбор модели в поле ввода чата — все участники используют закреплённую модель',
      selectModel: 'Выберите модель',
      noChatModels: 'Нет диалоговых моделей. Добавьте модель чата перед включением блокировки',
      saved: 'Настройка закрепления модели сохранена',
      saveFailed: 'Не удалось сохранить, попробуйте снова',
    },
    copySuffix: ' копия',
  ...
  chat: {
    ...
    exportPdfSuccess: 'PDF экспортирован',
  ...
  auth: {
    ...
    kingdeeLogin: 'Войти через Kingdee Cosmic',
    kingdeeLoginFailed: 'Не удалось перейти к Kingdee Cosmic',
  ...
  knowledgeBase: {
    ...
    moveTargetSection: 'Выберите целевую базу знаний',
    batchMoveSubtitle: 'Переместить выбранные документы ({count}) в другую базу знаний (перемещаются только документы с завершённым разбором)',
    batchMoveNoCompleted: 'Ни один из выбранных документов не завершил разбор — перемещать нечего',
    batchMoveSkippedNotCompleted: 'Пропущено документов без завершённого разбора: {count}',
    batchDownload: 'Скачать всё',
