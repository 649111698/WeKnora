export default {
  ...
  tenantSSO: {
      ...
      kingdee: {
        title: '金蝶苍穹单点登录',
        description: '以金蝶苍穹统一门户作为身份源登录本空间（苍穹 V6.0.015+，OAuth2.0 授权码模式）。苍穹侧在「系统服务云 → OpenAPI → 第三方应用」登记系统编码并授权 authen/getUserInfo 接口；员工从苍穹门户的免登菜单进入本系统，首次登录自动创建访客账号并加入本空间。',
        baseUrl: '苍穹站点地址',
        baseUrlPlaceholder: '例如 https://kde.erp.com（不带末尾斜杠）',
        appClientId: '系统编码（app_client_id）',
        appClientIdPlaceholder: '苍穹「第三方应用」的系统编码',
        appSecret: 'appSecret',
        appSecretPlaceholder: '苍穹「认证凭证」自定义的 appSecret',
        appSecretHint: '独立部署必须填写：与下方两项一起用于换取 access_token 后调用用户接口。',
        proxyUsername: '应用代理的用户名（username）',
        proxyUsernamePlaceholder: '苍穹「获取 Token 示例」中的 username',
        accountId: '数据中心 id（accountId）',
        accountIdPlaceholder: '苍穹「获取 Token 示例」中的 accountId',
        tokenModeHint: '以上三项（appSecret / username / accountId）在苍穹「第三方应用 → 获取 Token 示例」中可查看；独立部署的第三方必须配置，缺一项会退回 code 直连模式（多数私有化部署会被拒绝）。',
        callbackUrl: 'SSO 可信白名单回调地址',
        menuUrl: '苍穹门户免登链接',
        menuUrlHint: '配置到苍穹「菜单 / 快速发起 / 外部链接」，已登录苍穹的用户点击即免登进入本系统，默认落在对话页。',
        menuUrlFrameHint: '若苍穹以 iframe 内嵌该链接，需在部署侧为前端容器设置环境变量 FRAME_ALLOWED_ORIGINS=苍穹门户地址（如 https://portal.example.com），否则会被主站的防嵌策略（X-Frame-Options）拦截；新窗口打开则无需设置。',
  ...
  tenantMember: {
    ...
    directCreate: {
      button: '新增成员',
      dialogTitle: '新增成员（直接创建账号）',
      nameLabel: '姓名',
      namePlaceholder: '输入真实姓名',
      nameRequired: '姓名需 2-50 个字符',
      usernameLabel: '用户名',
      usernamePlaceholder: '2-50 个字符',
      usernameRequired: '用户名需 2-50 个字符',
      passwordLabel: '初始密码',
      passwordPlaceholder: '8-32 位，含字母和数字',
      passwordPolicy: '密码需 8-32 位且同时包含字母和数字',
      submit: '创建',
      success: '成员已创建并加入空间',
      failed: '创建成员失败，请重试',
    ...
    editName: {
      ...
      label: '姓名',
      placeholder: '输入真实姓名',
      required: '姓名需 2-50 个字符',
      saved: '姓名已更新',
      saveFailed: '更新姓名失败，请重试',
    },
    listTitle: '空间成员',
    loading: '正在加载成员…',
    retry: '重试',
  ...
  auth: {
    ...
    kingdeeLogin: '使用金蝶苍穹登录',
    kingdeeLoginFailed: '苍穹登录跳转失败',
    username: '用户名',
    email: '邮箱',
    password: '密码',
  ...
  knowledgeBase: {
    ...
    moveTargetSection: '选择目标知识库',
    batchMoveSubtitle: '将选中的 {count} 个文档移动到目标知识库（仅已完成解析的文档会被移动）',
    batchMoveNoCompleted: '选中的文档均未完成解析，无法移动',
    batchMoveSkippedNotCompleted: '已跳过 {count} 个未完成解析的文档',
    batchDownload: '批量下载',
    parsingFailed: '解析失败',
