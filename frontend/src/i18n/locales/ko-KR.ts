export default {
  ...
  tenantSSO: {
      ...
      kingdee: {
        ...
        description: 'Kingdee Cosmic 포털을 인증 소스로 사용합니다(Cosmic V6.0.015+, OAuth2.0 인증 코드). Cosmic 측 "시스템 서비스 클라우드 → OpenAPI → 제3자 애플리케이션"에 시스템 코드를 등록하고 authen/getUserInfo API를 승인하세요. 직원은 Cosmic 포털 메뉴를 통해 진입하며 첫 로그인 시 게스트 계정이 자동 생성됩니다.',
        baseUrl: 'Cosmic 사이트 주소',
        baseUrlPlaceholder: '예: https://kde.erp.com (끝 슬래시 없음)',
        appClientId: '시스템 코드(app_client_id)',
        appClientIdPlaceholder: 'Cosmic 제3자 애플리케이션의 시스템 코드',
        appSecret: 'appSecret',
        appSecretPlaceholder: 'Cosmic 인증 증명의 사용자 지정 appSecret',
        appSecretHint: '독립 배포에서는 필수입니다: 아래 두 항목과 함께 access_token으로 교환한 뒤 사용자 API를 호출합니다.',
        proxyUsername: '앱 프록시 사용자명(username)',
        proxyUsernamePlaceholder: 'Cosmic 「토큰 예시 가져오기」의 username',
        accountId: '데이터 센터 id(accountId)',
        accountIdPlaceholder: 'Cosmic 「토큰 예시 가져오기」의 accountId',
        tokenModeHint: '위 세 항목(appSecret / username / accountId)은 Cosmic 「서드파티 앱 → 토큰 예시 가져오기」에서 확인할 수 있습니다. 독립 배포의 서드파티는 반드시 설정해야 하며, 하나라도 비면 code 직접 모드로 되돌아가 대부분의 사설 배포에서 거부됩니다.',
  ...
  tenantMember: {
    ...
    directCreate: {
      button: '멤버 생성',
      dialogTitle: '멤버 생성(계정 직접 생성)',
      nameLabel: '이름',
      namePlaceholder: '실명을 입력하세요',
      nameRequired: '이름은 2-50자여야 합니다',
      usernameLabel: '사용자 이름',
      usernamePlaceholder: '2-50자',
      usernameRequired: '사용자 이름은 2-50자여야 합니다',
      passwordLabel: '초기 비밀번호',
      passwordPlaceholder: '8-32자, 문자와 숫자 포함',
      passwordPolicy: '비밀번호는 8-32자이며 문자와 숫자를 각각 1개 이상 포함해야 합니다',
      submit: '생성',
      success: '멤버가 생성되어 워크스페이스에 추가되었습니다',
    ...
    editName: {
      ...
      required: '이름은 2-50자여야 합니다',
      saved: '이름이 업데이트되었습니다',
      saveFailed: '이름 업데이트에 실패했습니다. 다시 시도해주세요',
    },
    listTitle: '워크스페이스 멤버',
    loading: '멤버를 불러오는 중…',
    retry: '다시 시도',
  ...
  auth: {
    ...
    kingdeeLogin: 'Kingdee Cosmic로 로그인',
    kingdeeLoginFailed: 'Kingdee Cosmic 이동 실패',
    username: '사용자명',
    email: '이메일',
    password: '비밀번호',
  ...
  knowledgeBase: {
    ...
    moveTargetSection: '대상 지식 베이스 선택',
    batchMoveSubtitle: '선택한 {count}개 문서를 다른 지식 베이스로 이동합니다(구문 분석이 완료된 문서만 이동됨)',
    batchMoveNoCompleted: '선택한 문서 중 구문 분석이 완료된 문서가 없어 이동할 수 없습니다',
    batchMoveSkippedNotCompleted: '구문 분석이 완료되지 않은 문서 {count}개를 건너뛰었습니다',
    batchDownload: '모두 다운로드',
    parsingFailed: '파싱 실패',
